...
<?php if (have_rows('main')) : while (have_rows('main')) : the_row();
  ...
  ?>
<section id="<?php echo esc_attr($id); ?>" class="<?php echo esc_attr($className); ?> c-hero flex flex-col w-full items-center p-0 lg:pt-5 lg:pb-0 px-4 md:px-10 bg-white h-[calc(100vh-165px)] bg-cover bg-no-repeat bg-[position:50%_20%]" fetchpriority=high
...
<?php if (have_rows('brands')) : while (have_rows('brands')) : the_row(); ?>
  ...
  <section class="c-brands pt-[40px] px-4 md:px-10 pb-0">
