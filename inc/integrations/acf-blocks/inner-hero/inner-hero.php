...
<?php if(get_sub_field('select_banner_height') == 'Full'): ?>
    ...
    <section id="<?php echo esc_attr($id); ?>" class="<?php echo esc_attr($className); ?> w-full px-4 md:px-10 bg-center bg-cover bg-no-repeat bg-[rgba(0,0,0,0.35)] bg-blend-overlay" style="--bg-desktop: url('<?php echo $bg_desktop_img; ?>');<?php if($bg_mobile_image): ?>--bg-mobile: url('<?php echo $bg_mobile_image; ?>');<?php endif; ?>">
      <div class="gap-10 w-full wrapper h-[calc(100vh-185px)] py-[80px] md:py-[118px] flex items-end">
