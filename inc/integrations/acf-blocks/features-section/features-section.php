  ...
  <section id="<?php echo esc_attr($id); ?>" class="<?php echo esc_attr($className); ?> our-values-sec px-4 md:px-10 py-[60px] lg:py-[80px] xl:py-[120px] overflow-hidden <?php echo $bg_color_class; ?><?php echo $pt_class; ?><?php echo $pb_class; ?>">
	  ...
	  <?php if($title_row_1 || $title_row_2 ): ?>
		...
		<?php if($lists): ?>
			...
			<div class="flex flex-wrap xl:gap-x-[70px] md:gap-y-[90px] lg:gap-x-[30px] gap-x-[20px] sm:gap-y-[40px] gap-y-[40px] justify-center two-list-fred">
				<?php foreach($lists as $list): 
					...
					<div class="flex flex-row md:gap-[20px] gap-[16px] <?php echo $width_class; ?>">
						<div class="flex flex-col flex-1 red-line pb-[25px] relative">
							<?php if($heading): ?>
								<h3 class="text-[clamp(20px,2.6vw,28px)] leading-[clamp(28px,3.2vw,36px)] tracking-[-2%] text-[#262626] font-heading font-bold mb-[12px] md:pt-[10px] pt-[8px] md:pl-[35px] pl-[30px]"><?php echo $heading; ?></h3>
							<?php endif; ?>
							<?php if($content): ?>
								<div class="text-[18px] leading-[26px] text-[#525252] font-body tracking-[0px]"><?php echo $content; ?></div>
							<?php endif; ?>
							
							<img class="w-[18px] md:w-[23px] md:h-[23px] absolute left-0 md:top-[14px] top-[10px]" src="<?= get_template_directory_uri() ?>/assets/imgs/red-arrow-feature-sec.svg" alt="arrow">
						</div>
					</div>
				<?php endforeach; ?>
			</div>
