...
<aside id="sidebar-filter" class="flex flex-col w-full md:w-[220px] lg:w-[280px] xl:w-[360px] flex-shrink-0 gap-4 mt-[25px]" aria-label="Filter categories">
   <div class="flex justify-between items-center w-full mb-[15px]">
     ...
     <a href="/work/" class="hidden md:inline-block font-[Poppins] font-medium text-[16px] leading-[24px] text-[#525252] underline hover:no-underline hover:text-accent transition-colors">Reset Filters</a>
       ...
       <?php if (!empty($filter_groups)) : ?>
          <ul class="space-y-[28px]">
            <?php foreach ($filter_groups as $group) :
              ...
              $active_is_acf_child = $active_term_id > 0 && in_array($active_term_id, $acf_child_ids, true);
            ...
            ?>
              <li class="<?php echo $has_children ? 'has-child' : ''; ?>">
                ...
                <?php if ($has_children) : ?>
                  <ul class="child-list space-y-[28px] pl-4 pr-[20px] <?php echo $is_open ? 'is-open' : ''; ?>">
